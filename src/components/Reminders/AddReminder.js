...
const AddReminder = ({ history }) => {
  ...
  return error
    ? <p data-testid="error" className="red tc lh-title f1">Ooops there was an error, please try again.</p>
    : (
      <form onSubmit={onSubmit} className="flex flex-column p3 pb6">
        ...
        <input data-testid="title" required className="mb4 pa2" onChange={onChange} value={state.current.title || ''} type="text" id="title" />
        ...
        {categories && (
          <div className="flex flex-column p3">
            ...
            <select data-testid="categories-menu" required onChange={e => onCategoryChange(e, categories)} className="mb4" id="category">
              <option value=''>Choose a category</option>
              {categories && categories.map((category, i) => (
                <option data-testid="categories-menu-item" key={i} value={category.id}>{category.categoryName}</option>
              ))}
            </select>
          </div>
        )}
        ...
        {providers && (
          <div className="flex flex-column p3">
            ...
            <select data-testid="providers-menu" required onChange={onChange} className="mb4" id="provider">
              <option value=''>Choose a Provider</option>
              {providers.map((provider, i) => (
                <option data-testid="providers-menu-item" key={i} value={provider.company.companyName}>{provider.company.companyName}</option>
              ))}
            </select>
          </div>
        )}
        ...
        <DatePicker data-testid="date" className="mb4 w-100 pa2" selected={state.current.endDate || ''} onChange={onDateChange} />

        <label className="pb2" htmlFor="noticePeriod">
          Notice period <span className="gray">(in days)</span>
        </label>
        <input data-testid="notice-period" required className="mb4 pa2" onChange={onChange} value={state.current.noticePeriod || ''} type="number" id="noticePeriod" />

        <input data-testid="submit" className="pointer grow bg-light-green white pa3 bn b f4 br2" required type="submit" value={actionText} />
      </form >
    )
}
